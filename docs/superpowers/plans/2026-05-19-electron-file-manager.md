...
这是一个典型的桌面应用架构，采用左右分栏的界面设计，并通过进程间通信（IPC）将系统操作与用户界面解耦。

### 界面布局
*   **左侧：文件树**
    *   提供可视化的项目文件目录结构。
    *   支持文件与文件夹的浏览、创建、删除、重命名等基础操作。
*   **右侧：Monaco Editor**
    *   集成功能强大的代码编辑器（VS Code 同款）。
    *   提供语法高亮、智能提示、代码折叠等专业的编辑体验。

### 进程通信机制
*   **主进程**
    *   负责所有与操作系统相关的**文件系统操作**（如读取、写入、监听文件变化）。
    *   拥有完整的系统访问权限，运行在 Node.js 环境中。
*   **渲染进程**
    *   负责**用户界面（UI）** 的渲染与交互，即上述的文件树和编辑器。
    *   运行在 Chromium 环境中，出于安全考虑，其系统访问能力受限。
*   **IPC 桥接**
    *   作为主进程与渲染进程之间的**通信桥梁**。
    *   渲染进程通过 IPC **发送请求**（如“读取某文件”），主进程接收请求并执行操作，再将**结果返回**给渲染进程进行展示。
    *   这种设计确保了应用的安全性和稳定性。
